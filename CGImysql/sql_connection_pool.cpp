...
void connection_pool::init(string url, string User, string Passward, string DataBaseName, int port, int MaxConn,
                           int close_log) {
    this->m_url = url;
    this->m_User = User;
    ...
    for (int i = 0; i < MaxConn; i++) {
        ...
        conn = mysql_real_connect(conn, url.c_str(), User.c_str(), Passward.c_str(), DataBaseName.c_str(), port,
                                  nullptr, 0);
        if (!conn) {
            cout << "error" << endl;                          //需要修改
            exit(1);
